...
const PurchaseScreen = (props) => {
  ...
  const fetchParties = () => {
    ...
    axios.get(partyUrl+"/all").then(resp => { 
      setParties([])
      setParties(resp.data.data.filter(party => (party.isDeleted!==true || party.isDeleted===null) && party.type==="PROVIDER"))  
      setFilteredData(resp.data.data.filter(party => (party.isDeleted!==true || party.isDeleted===null) && party.type==="PROVIDER"))  
      setLoading(false)
    })
    .catch(err => {
  ...
  const clearData = () => {
    ...
    document.getElementById("hsnCode-input").value=""
  }
  ...
  return (
    <div className='Screen'>
        <Header loading={loading} title='Save Purchase Bill'/>
        <Modal  displayed={modalDisplay} 
                onConfirm={onConfirmOrder}
                onCancel={onCancelOrder}
                title="Confirm Purchase Order"
                message="Do you want to save this order ? "
        />
        <div className='billForm row'>
          <h5 className='form-label text-center'>Seller Name </h5><hr/>
          ...
          <div className='order-area compact-display mb-4' >
            ...
            <div className='row receipt-row'>
              ...
              <div className='col-lg-4 col-md-4 col-12 mb-4'>
                <h6>Bill Number</h6>
                <input type="text" onChange={e=>{setOrder({...order, billNumber: e.target.value})}} className="form-control" placeholder='Enter Bill number' />
              </div>

              <div className='col-lg-4 col-md-4 col-12 mb-4'>
                <h6>HSN Code</h6>
                <input id="hsnCode-input" type="text" value={order.hsnCode ? order.hsnCode: ""} onChange={e=>{setOrder({...order, hsnCode: e.target.value})}} className="form-control" placeholder='Enter hsnCode number' />
              </div>
            </div>
            ...
            <div className='row receipt-row'>
              <div className='col-lg-12 col-md-12 col-12'>
                <button className='btn btn-success form-btn' style={{width: "100%"}} onClick={onGenerateBill} >Save Purchase</button>
              </div>
            </div>
